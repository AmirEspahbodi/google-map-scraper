...
class ScrapDataBo:
    ...
    async def __extract_listings(browser_page: BrowserPage):
        try:
            ...
            for listing in listings:
                ...
                if tree is not None:
                    ...
                    if phone and title and short_address:
                        if phone in result:
                            result[f"{title} - {phone} - {short_address}"+f"_{len(result[f"{title} - {phone} - {short_address}"])}"] = listing
                        else:
                            result[f"{title} - {phone} - {short_address}"] = listing
                    elif phone and title:
                        if phone in result:
                            result[f"{title} - {phone}"+f"_{len(result[f"{title} - {phone}"])}"] = listing
                        else:
                            result[f"{title} - {phone}"] = listing
                    elif phone and short_address:
                        if phone in result:
                            result[f"{phone} - {short_address}"+f"_{len(result[f"{phone} - {short_address}"])}"] = listing
                        else:
                            result[f"{phone} - {short_address}"] = listing
                    elif title and short_address:
                        if f"{title} - {short_address}" in result:
                            result[f"{title} - {short_address}"+f"_{len(result[f"{title} - {short_address}"])}"] = listing
                        else:
                        ...
                        if short_address:
                            if short_address in result:
                                result[short_address+f"_{len(result[short_address])}"] = listing
                            else:
            ...
            print(f"error in ScrapDataBo in __extract_listings function\nerror: {e}\npage={browser_page.name}")
